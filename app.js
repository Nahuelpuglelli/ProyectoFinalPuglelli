...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('submit-user').addEventListener('click', () => {
        ...
        if (nombre && !isNaN(edad) && edad >= 18) {
            ...
            Swal.fire('Por favor, ingrese un nombre válido y asegúrese de ser mayor de edad.');
        }
    });
    ...
    document.getElementById('products').addEventListener('click', (event) => {
        if (event.target.classList.contains('add-to-cart')) {
            ...
            if (producto && !isNaN(productQuantity) && productQuantity > 0 && productQuantity <= producto.stock) {
                ...
                Swal.fire('Cantidad inválida o no hay suficiente stock.');
            }
        }
    });
    ...
    document.getElementById('checkout').addEventListener('click', () => {
        if (carrito.length > 0) {
            Swal.fire(`El total de su compra es $${totalCompra}. ¡Gracias por su compra!`);
            carrito = [];
            totalCompra = 0;
